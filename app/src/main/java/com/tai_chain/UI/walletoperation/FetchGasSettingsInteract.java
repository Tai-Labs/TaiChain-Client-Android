...
import com.tai_chain.bean.ConfirmationType;
...
import static com.tai_chain.base.Constants.GAS_LIMIT_MIN;
import static com.tai_chain.base.Constants.GAS_PER_BYTE;
...
public class FetchGasSettingsInteract {
    ...
    public FetchGasSettingsInteract(EthereumNetworkRepository networkRepository) {
        this.networkRepository = networkRepository;

        this.currentChainId = networkRepository.getDefaultNetwork().chainId;

        cachedGasPrice = new BigInteger(Constants.DEFAULT_GAS_PRICE);
    ...
    public Single<GasSettings> fetch(ConfirmationType type) {
        ...
        return Single.fromCallable( () -> {
            BigInteger gasLimit = new BigInteger(Constants.DEFAULT_GAS_LIMIT);
            if (type == ConfirmationType.ETH) {
                gasLimit = new BigInteger(Constants.DEFAULT_GAS_LIMIT_FOR_ETH);
            } else if (type == ConfirmationType.ERC20) {
                gasLimit = new BigInteger(Constants.DEFAULT_GAS_LIMIT_FOR_TOKENS);
            }
...
//        return repository.getGasSettings(forTokenTransfer);
    ...
    private void fetchGasPriceByWeb3() {
        MyLog.d("fetchGasPriceByWeb3 start");
        ...
        try {
            ...
            if (price.getGasPrice().compareTo(BalanceUtils.gweiToWei(BigDecimal.ONE)) >= 0)
            {
                ...
                MyLog.d("FetchGasSettingsInteract web3 price:" +  price.getGasPrice());
    ...
    public Single<GasSettings> getGasSettings(byte[] transactionBytes, boolean isNonFungible) {
        return Single.fromCallable( () -> {
            ...
            if (transactionBytes != null) {
                if (isNonFungible)
                {
                    gasLimit = new BigInteger(Constants.DEFAULT_GAS_LIMIT_FOR_NONFUNGIBLE_TOKENS);
                }
                else
                {
                    gasLimit = new BigInteger(Constants.DEFAULT_GAS_LIMIT_FOR_TOKENS);
    ...
    public Single<GasSettings> fetchDefault(boolean tokenTransfer, NetworkInfo networkInfo) {
        return Single.fromCallable(() -> {
            BigInteger gasPrice = new BigInteger(Constants.DEFAULT_GAS_PRICE);
            BigInteger gasLimit = new BigInteger(Constants.DEFAULT_GAS_LIMIT);
            if (tokenTransfer) {
                gasLimit = new BigInteger(Constants.DEFAULT_GAS_LIMIT_FOR_TOKENS);
            }
